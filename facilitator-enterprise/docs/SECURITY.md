...
This document describes the security features available in the enterprise facilitator and how to configure them for production deployments.

## Overview

The enterprise facilitator includes comprehensive protection mechanisms to prevent abuse and secure your deployment:
...
All security features are **optional** and **backwards compatible** with existing deployments. If you provide no `config.toml` file, the facilitator uses sensible defaults.
...
All security configuration lives in `config.toml` (the enterprise TOML config) and environment variables. See [Configuration Guide](CONFIGURATION.md) for the full reference.
...
- Localhost whitelist by default
...
### 4. Request Validation and Abuse Detection
...
5. **Monitor security logs**
   ```bash
   RUST_LOG=info facilitator-enterprise 2>&1 | grep -E "(banned|blocked|unauthorized|suspicious)"
   ```
...
If running a public facilitator:
...
```dockerfile
FROM facilitator-enterprise:latest
...
- [Configuration Guide](CONFIGURATION.md) - Full security configuration reference
